...
function handle_get_events($config, $pythonLogsDir) {
    ...
    if (file_exists($security_file)) {
        ...
        foreach ($pythonEvents as &$event) {
            ...
            // Ensure source_ip is set for consistency
            if (!isset($event['source_ip']) && isset($event['source'])) {
                $event['source_ip'] = $event['source'];
            }
        }
    ...
    // Load raw logs from captured_logs directory
    $raw_logs_by_ip = [];
    if (is_dir($pythonLogsDir)) {
            ...
            if (is_array($data)) {
                ...
                foreach ($data as $entry) {
                    ...
                    if ($ip) {
                        ...
                        if (!isset($raw_logs_by_ip[$ip])) {
                            $raw_logs_by_ip[$ip] = [];
                        }
                        
                        $raw_logs_by_ip[$ip][] = [
                            'timestamp' => $entry['timestamp'] ?? $entry['date'] ?? date('Y-m-d H:i:s'),
                            'type' => $log_type,
                            'line' => $log_line
                        ];
                    }
    ...
    // Attach raw logs to events based on source_ip
    foreach ($events as &$event) {
        $event_source = $event['source_ip'] ?? $event['source'] ?? null;
        if ($event_source && isset($raw_logs_by_ip[$event_source])) {
            $raw_logs = $raw_logs_by_ip[$event_source];
            // Convert to simple line format for display
            $event['raw_logs'] = array_map(function($log) {
                return $log['line'];
            }, $raw_logs);
        } else {
            $event['raw_logs'] = [];
